...
namespace PartsPageClasses
{
    public class Oil : IOil
    {
        ...
        public Oil()
        {
            ...
            //_driver.Navigate().GoToUrl("http://localhost:5001/Store/Browse?CategoryId=5");
            _driver = DriverFactory.GetDriver("http://localhost:5000/Store/Browse?CategoryId=5");
        ...
        public string GetPartUrl(int partIndex)
        {
            var parentDiv = _driver.FindElement(By.CssSelector(".row"));
            var childElements = parentDiv.FindElements(By.CssSelector(".col-lg-3.col-md-3.col-sm-3.col-xs-12"));
            ...
            if (partIndex >= 0 && partIndex < childElements.Count)
            {
                try
                {
                    // In PartsUnlimited, each part card has an <a> that links to details
                    var linkElement = childElements[partIndex].FindElement(By.TagName("a"));
