...
namespace PartsPageClasses
{
        ...
        public class More : IMore
        {
            ...
            public More()
            {
                ...
                _driver.Navigate().GoToUrl("http://localhost:5000/Store");
            }
            ...
            public string GetPartName(int partIndex)
            {
                ...
                // Loop through each child element and extract the text inside the <h5> tag
                foreach (var element in childElements)
                {
                    try
                    {
                        // Find the <h5> tag inside this element and get its text
                        var h5Tag = element.FindElement(By.TagName("h5"));
                        h4Texts.Add(h5Tag.Text);
                    }
                    catch (NoSuchElementException)
                    {
                        // If no <h5> tag is found, handle the exception
                        Console.WriteLine("No <h5> tag found in one of the elements.");
                    }
                }
